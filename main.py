...
def user_login():
    ...
    if request.method == 'POST':
        ...
        if (data=="no"):
            ...
            return render_template('index.html',flash_message=True,msg="Login Failed")
        else:

            msg='Success'
            session['user'] =n
            items = mm.select_direct_query(
                "select name,aadhar_number,pan_number,image from user_details where email='" + str(n) + "'")
            for x in items:
                username = x[0]
                aadhar_number = x[1]
                pan_number = x[2]
...
def bank_login():
    ...
    if request.method == 'POST':
        ...
        data=len(data)
        if data==0:
            return render_template('bank_login.html',flash_message=True,msg="Login Failed")
        else:
            msg='Success'
            session['bank'] =n
            session['pic'] ="bank.png"
...
def user_register():
    if request.method == 'POST':
        ...
        kk=mm.select_direct_query("select * from user_details where email='"+str(email)+"'")
        if kk:
            return render_template('user_register.html', msg="Email Already Registered")



        maxin = mm.find_max_id("user_details")
        qq = "insert into user_details values('" + str(maxin) + "','" + str(name) + "','" + str(contact) + "','" + str(
            email) + "','" + str(address) + "','" + str(password) + "','" + str(aadhar_number) + "','" \
             + str(pan_number) + "','0','"+str(gender)+"','"+str(secure_filename(f.filename))+"')"
        result = mm.insert_query(qq)

        if (result == 1):
            return render_template('index.html',flash_message=True,data="Registration Success")
        else:
...
def bank_register():
    if request.method == 'POST':
        ...
        if (result == 1):
            return render_template('bank_login.html', msg="Registration Success")
        else:
...
def user_create_account():
    ...
    uname=session['user']
    items=mm.select_direct_query("select name,aadhar_number,pan_number from user_details where email='"+str(uname)+"'")
    for x in items:
        username=x[0]
        aadhar_number=x[1]
        pan_number=x[2]
...
def user_deposit():
    ...
    if request.method == 'POST':
        ...
        return render_template('user_deposit.html',items=data,flash_message=True,data="Deposite Success")
...
def user_transcation():
    ...
    if request.method == 'POST':
        ...
        if(a1>=b1):
            ...
            ##########################################################################################
            return render_template('user_transcation.html', items=data,message='Success',flash_message=True,data="Transcation Success")
        else:
...
def user_received_1(id):
    ...
    if rrr=="refund":
        ...
        qry = "update mini_statement set ip='" + str(IPAddr) + "',mac='" + str(mac) + "',latitude='" + str(
            latitude) + "',longitude='" + str(longitude) + "' where verification='" + str(id) + "'"
        mm.insert_query(qry)
...
@app.route("/user_modified",methods = ['GET', 'POST'])
def user_modified():
    user=session['user']
    q=("SELECT  * from mini_statement where user='" + str(user) + "' and status IN ('Debit Refund','Credit Refund')")
    data=mm.select_direct_query(q)


    return render_template('user_modified.html',items=data)
...
def bank_user():
    ...
    q=("SELECT user_details.id,name,contact,email,address,aadhar_number,pan_number  from user_details,account_details where user_details.email=account_details.user and account_details.bank='"+str(bank)+"'")
    print(q)
    data=mm.select_direct_query(q)
...
def bank_transaction():
    n=session['bank']
    q=("SELECT * from mini_statement where bank='"+ str(n) +"'  and  status NOT IN ('Debit Refund','Credit Refund')")
    data=mm.select_direct_query(q)
    return render_template('bank_transaction.html',items=data)

@app.route("/bank_modified",methods = ['GET', 'POST'])
def bank_modified():
    n=session['bank']
    q=("SELECT * from mini_statement where bank='"+ str(n) +"' and  status IN ('Debit Refund','Credit Refund')")
    data=mm.select_direct_query(q)
    return render_template('bank_modified.html',items=data)

@app.route("/admin_modified",methods = ['GET', 'POST'])
def admin_modified():
    q=("SELECT * from mini_statement where  status IN ('Debit Refund','Credit Refund')")
    data=mm.select_direct_query(q)
    return render_template('admin_modified.html',items=data)

if __name__ == '__main__':
    app.run(debug=True, use_reloader=True,port=4050,host='0.0.0.0')
